# !pip install -qU langchain-aws langchain langchain-classic langgraph langchain-community pypdf PyPDF2 langchain-text-splitters langchain-google-community unstructured[pdf] rich faiss-cpu
...
unified_persist_directory = "unified_faiss_db"
faiss_index_path = os.path.join(unified_persist_directory, "index.faiss")
...
if not db_exists:
    ...
    for filename in os.listdir(structured_data_dir):
        if filename.endswith(".pdf"):
            ...
            if filename not in DOCUMENT_TYPES_CONFIG:
                print(f"[!] No configuration found for {filename}. Skipping...")
                continue
    ...
    print(f"Creating FAISS index at {unified_persist_directory}...")
    db_unified = FAISS.from_documents(
        documents=all_chunks,
        embedding=embeddings
    )
    db_unified.save_local(unified_persist_directory)
    print(f"[+] FAISS index created and saved at {unified_persist_directory} with {len(all_chunks)} documents.")
else:
    # FAISS index already exists, just load it
    print(f"\n[+] Loading existing FAISS index from {unified_persist_directory}...")
    db_unified = FAISS.load_local(
        folder_path=unified_persist_directory,
        embeddings=embeddings,
        allow_dangerous_deserialization=True
    )
    print(f"[+] FAISS index loaded with {db_unified.index.ntotal} documents.")
